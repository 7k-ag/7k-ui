...
>(function AccordionItem({ children, className, ...props }, forwardedRef) {
  return (
    <RadixAccordion.Item
      {...props}
      className={tw(
        "group overflow-hidden bg-gray-20 first:mt-0 first:rounded-t-2xl last:rounded-b-2xl focus-within:relative focus-within:z-10 outline-none",
        className,
      )}
      ref={forwardedRef}
    >
  ...
  return (
    <RadixAccordion.Header className="flex font-bold text-sm/normal">
      <RadixAccordion.Trigger
        {...props}
        className={tw(
          "w-full px-6 py-4 outline-none rounded-2xl flex justify-between text-left focus-visible:shadow-[inset_0_0_0_1px]",
          className,
        )}
        ref={forwardedRef}
      >
