...
import { PanInfo, m, useAnimation } from "framer-motion";
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Overlay
    className={tw("fixed inset-0 z-modal bg-black/20", className)}
    {...props}
    ref={ref}
  />
));
...
>(
  (
    ...
    const onPan = React.useCallback(
      (e: any, panInfo: PanInfo) => {
        if (!animation) return;
        const { x, y } = panInfo.offset;
        if (e.cancelBubble) return;
        switch (side) {
          case "left":
            control.set({ x: Math.min(x, 0) });
            break;
          case "right":
            control.set({ x: Math.max(x, 0) });
            break;
          case "top":
            control.set({ y: Math.min(y, 0) });
            break;
          case "bottom":
            control.set({ y: Math.max(y, 0) });
            break;
        }
      },
    ...
    const onPanEnd = React.useCallback(
      (_: any, panInfo: PanInfo) => {
        if (!animation) return;
        const { x: vx, y: vy } = panInfo.velocity;
        switch (true) {
          case side === "left" && vx < -200:
          case side === "right" && vx > 200:
          case side === "top" && vy < -200:
          case side === "bottom" && vy > 200:
            onOpenChange?.(false);
            break;
          default:
            control.start("open");
      ...
      [animation, control, onOpenChange, side],
    );
    React.useEffect(() => {
      control.start("open");
    }, [control]);
    return (
      <SheetPortal forceMount={props.forceMount}>
        ...
        <SheetPrimitive.Content
          ref={ref}
          className={tw(
            "fixed z-modal bg-skin-card p-6 shadow-lg outline-none overflow-auto",
        ...
        >
          ...
          >
            <m.div
              className={tw(
                "absolute bg-gray-20 rounded-full",
...
>(({ className, ...props }, ref) => (
  ...
  >
    <Button
      className="btn-gray-20 dark:btn-gray-inverted-20 font-bold text-sm/none h-6 w-12 rounded-lg"
      innerClassName="h-6"
      size="xs"
    >
      ESC
