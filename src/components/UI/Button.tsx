...
const sizeClasses = {
  lg: "h-13 font-bold text-lg rounded-xl hover:shadow-soft-2",
  md: "h-10 font-bold text-base rounded-[10px] hover:shadow-soft-2",
  sm: "h-8 font-bold text-xs/normal rounded-md hover:shadow-soft-2",
  ...
  return (
    <button type="button" {...rest} className={btn} ref={ref}>
      <span className={inner}>
        {loading && (
          <span className="shink-0 mr-[1ch] inline-block w-[2ch] h-[2ch] rounded-full border-[0.35ch] border-t-transparent border-current animate-spin"></span>
        )}
        {children}
      </span>
    </button>
